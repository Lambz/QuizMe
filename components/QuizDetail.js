...
export default function QuizDetail({route, navigation}) {
    ...
    return(
        <ScrollView>
            <Image style={styles.image} source={getRandomImage(quiz.typeOfQuiz)}></Image>
            <Text style={styles.header}>{quiz.name}</Text>
            <View style={styles.details}>
                <Text style={styles.category}><Text style={{fontWeight: "bold"}}>Category</Text>: {categories[quiz.typeOfQuiz].label}</Text>
                <Text style={styles.category}><Text style={{fontWeight: "bold"}}>Questions</Text>: {quiz.questions.length}</Text>
            </View>
            <Text>{message}</Text>
            ...
            <View style={[styles.descriptionView, styles.leaderboardColor]}>
                ...
                <View style={styles.details}> 
                    ...
                    <Text style={{fontWeight:"bold", fontSize:15}}>Player</Text>
                    <Text style={{fontWeight:"bold", fontSize:15}}>Score</Text>
                </View>
                <FlatList data={quiz.max_scores.sort((a,b) => (b-a))} renderItem={({item}) => (
